...
test('maps function keys to app actions', () => {
  assert.equal(getShortcutAction({ key: 'F8' }), ACTIONS.TOGGLE_APP_MENU);
  assert.equal(getShortcutAction({ key: 'F9' }), ACTIONS.QUIT);
  assert.equal(getShortcutAction({ key: 'F10' }), ACTIONS.TOGGLE_FULLSCREEN);
});

test('maps Shift+8, Shift+9 and Shift+0 to app actions', () => {
  assert.equal(getShortcutAction({ code: 'Digit8', key: '*', shiftKey: true }), ACTIONS.TOGGLE_APP_MENU);
  assert.equal(getShortcutAction({ code: 'Digit8', key: '8', shift: true }), ACTIONS.TOGGLE_APP_MENU);
...
test('ignores non-shortcut and modified digit input', () => {
  assert.equal(getShortcutAction({ code: 'Digit8', key: '8' }), null);
  assert.equal(getShortcutAction({ code: 'Digit9', key: '9' }), null);
  assert.equal(getShortcutAction({ code: 'Digit0', key: '0' }), null);
  assert.equal(getShortcutAction({ code: 'Digit8', key: '*', shiftKey: true, ctrlKey: true }), null);
...
test('exposes global accelerators for registration', () => {
  assert.deepEqual(
    getAcceleratorsForAction(ACTIONS.TOGGLE_APP_MENU),
    ['F8', 'Shift+8'],
  );
  assert.deepEqual(
  ...
  assert.equal(GLOBAL_SHORTCUTS.length, 6);
});
